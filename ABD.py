...
def calc_Q_laminate(Q_0, Q_0degraded, angles, active_plies, failed_once, n_plies):
    #The function creates a 3 x 3 x num of plies array of Q matricies for a laminate made out of identical plies
    ...
    #         true/false array stating whether a ply failed in transverse direction only (0.15 degradation rule is applied) ((by default all false))
    #         number of plies
...
def lamina_strains12(angles, abd, F, t, n_plies, active_plies):
    ...
    strain_xy_mid = abd @ F
    strain = np.zeros((2,3,n_plies)) # 2 for bottom and top values; 3 for x, y, shear; per ply
    ...
    for i in range(n_plies):
        if active_plies[i] == True:
            ...
            for j in range(2):
                ...
                if stressT >= 0:
                    matrix_tension = np.sqrt((stressTL/Stc)**2 + (1-snnL*Yt/Stc)**2 * (stressT/Yt)**2) + snnL*stressT/Stz
                    if matrix_tension >= 1:
                        if failed_once[i] == True:
                            active_plies[i] = False
                            break
...
if __name__ == '__main__':
    ...
    sym2 = sym1+sym1[::-1]
    sym3 = sym2+sym2[::-1]
    angles = np.pi/180 * np.array(sym3)
    ...
    for i in range(number):
        ...
        direction = np.array([ratios[0, i], ratios[1, i], 0.0, 0.0, 0.0, 0.0])
        ...
        while np.any(active_plies):
            ...
            strain = lamina_strains12(angles, abd, Fxy, t, n_plies, active_plies)
            stress = lamina_stress12(strain, Q_0, Q_0degraded, n_plies, failed_once)
        ...
        # fallback in case LPF wasn't written inside loop
        if np.isnan(last_ply_failure_NxNy[0, i]):
            last_ply_failure_NxNy[0, i] = Fxy[0]
            last_ply_failure_NxNy[1, i] = Fxy[1]
