...
def failure(stress, strain, E1, Ef, v12f, Xt, Xc, Yt, Yc, Stc, n_plies, active_plies1, failed_once1):
    ...
    Stz = 100.0  # example value for CFRP
...
if __name__ == '__main__': #Only Run this code if this file is run (and not if it is being imported)
    ...
    plt.figure(figsize=(7, 7))
    plt.scatter(first_ply_failure_NyNs[0, :], first_ply_failure_NyNs[1, :], label='FPF')
    plt.scatter(last_ply_failure_NyNs[0, :], last_ply_failure_NyNs[1, :], label='LPF')
    plt.axhline(0, linewidth=0.8)
    plt.axvline(0, linewidth=0.8)
    plt.xlabel('Ns [N/mm]')
    plt.ylabel('Ny [N/mm]')
