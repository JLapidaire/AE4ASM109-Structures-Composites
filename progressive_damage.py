...
### This file assumes symmetric laminate and only in plane loads,
# so the calculations can be simplified
...
def calc_A(Q_all, t, n_plies):
    ...
    #           number of plies
    # Output:   A matrix, can be used instead of full ABD if only in plane loads are applied,
...
def lamina_strains_inplane(angles, abd, F, n_plies, active_plies):
    ...
    #         inverted A matrix
    #         3x1 force intensity vector F (Nx, Ny, Ns) in N/mm
    #         thickness of the plies (assumes the same for all)
    #         number of plies
    # Outputs: 1 x 3 x n_plies array with 1, 2, and 12 strain values per ply

    strain_xy_mid = abd @ F
    strain = np.zeros((1, 3, n_plies))  # 1 per ply (constant throughout the ply since only in-plane loads);
    # 3 for 1, 2, and 12 strains; for the number of plies
...
def lamina_stress_inplane(strain, Q_0, Q_0degraded, n_plies, failed_once):
    ...
    # Inputs: 1 x 3 x n_plies array with 1, 2, and 12 strain values per ply
    #         Q_0 array per initial ply at 0 deg
    #         Q_0degraded per degraded ply at 0 deg
    #         n_plies number of plies
    #         true/false array stating whether a ply is able to carry loads (by default all true)
    # Outputs: 1x3xn_plies array with 1, 2, and 12 stress values for each ply
...
def failure(stress, strain, E1, Ef, v12f, Xt, Xc, Yt, Yc, Stc, n_plies, active_plies1, failed_once1):
    ...
    # Inputs: 1x3xn_plies array with 1, 2, and 12 stress values for each ply (in MPa);
    #         1x3xn_plies array with 1, 2, and 12 strain values per ply (unitless strain);
...
if __name__ == '__main__':  # Only Run this code if this file is run (and not if it is being imported)
    ...
    Yc = 220  # MPa
    Stc = 144.5  # MPa
    Ef = 230 * 10 ** 3  # MPa Toray T300 fibres
    v12f = 0.25
    base = [0, 45, -45, 90, 30]
    sym1 = (base) + (base)[::-1]
    sym2 = sym1 + sym1[::-1]
    ...
    Stz = np.sqrt(((1 + Yt / Yc) / (3 + 5 * Yt / Yc)) * Yt * Yc)
    print(Stz)

    S, Q_0 = inplane_matrices(E1, E2, v12, G12)

    # Computed once at the beginning, so that it doesn't have to be done at every step
    ...
    lpfstrainsNyNs = np.zeros((3, number))
    fpfstrainsNyNs = np.zeros((3, number))
    lpfstrainsNxNy = np.zeros((3, number))
    fpfstrainsNxNy = np.zeros((3, number))
    for i in range(number):
        ...
        Fxy = load_step * direction  # force intensity in N/mm
        ...
        while np.any(active_plies):
            ...
            if state_changed:
                ...
                if not fpf_recorded:
                    ...
                    current_strain = abd @ Fxy
                    fpfstrainsNxNy[:, i] = current_strain
                    fpf_recorded = True
                ...
                # if all plies are gone, record LPF at this load
                if not np.any(active_plies):
                    last_ply_failure_NxNy[0, i] = Fxy[0]
                    last_ply_failure_NxNy[1, i] = Fxy[1]
                    current_strain = abd @ Fxy
                    lpfstrainsNxNy[:, i] = current_strain
                    break

                # rebuild stiffness and analyze again at the SAME load
                Q_all = calc_Q_laminate(Q_0, Q_0degraded, angles, active_plies, failed_once, n_plies)
                ABD = calc_A(Q_all, t, n_plies)
                abd = np.linalg.inv(ABD)
    ...
    for i in range(number):
        ...
        # loading direction for this ratio
        direction = np.array([0.0, ratios[1, i], ratios[0, i]])
        ...
        while np.any(active_plies):
            ...
            if state_changed:
                # first state change = first ply failure
                if not fpf_recorded:
                    first_ply_failure_NyNs[0, i] = Fxy[2]
                    first_ply_failure_NyNs[1, i] = Fxy[1]
                    current_strain = abd @ Fxy
                    fpfstrainsNyNs[:, i] = current_strain
                    fpf_recorded = True
                ...
                if not np.any(active_plies):
                    last_ply_failure_NyNs[0, i] = Fxy[2]
                    last_ply_failure_NyNs[1, i] = Fxy[1]
                    current_strain = abd @ Fxy
                    lpfstrainsNyNs[:, i] = current_strain
                    break
    ...
    # The code was optimised strictly for the assignment task (multiple symmetries and only in plane loads)
    # We calculated the FPF and LPF of 1/8 of the stack, and now we need to multiply it back times 8
    last_ply_failure_NyNs *= 2 ** 3
    first_ply_failure_NyNs *= 2 ** 3
    ...
    # Close the curves
    first_ply_failure_NxNy = np.hstack((first_ply_failure_NxNy, first_ply_failure_NxNy[:, [0]]))
    last_ply_failure_NxNy = np.hstack((last_ply_failure_NxNy, last_ply_failure_NxNy[:, [0]]))
    first_ply_failure_NyNs = np.hstack((first_ply_failure_NyNs, first_ply_failure_NyNs[:, [0]]))
    last_ply_failure_NyNs = np.hstack((last_ply_failure_NyNs, last_ply_failure_NyNs[:, [0]]))

    plt.figure(figsize=(7, 7))
    plt.plot(first_ply_failure_NxNy[0, :], first_ply_failure_NxNy[1, :], label='FPF')
    plt.plot(last_ply_failure_NxNy[0, :], last_ply_failure_NxNy[1, :], label='LPF')
    ...
    plt.figure(figsize=(7, 7))
    plt.plot(first_ply_failure_NyNs[0, :], first_ply_failure_NyNs[1, :], label='FPF')
    plt.plot(last_ply_failure_NyNs[0, :], last_ply_failure_NyNs[1, :], label='LPF')
    plt.axhline(0, linewidth=0.8)
    plt.axvline(0, linewidth=0.8)
    plt.xlabel('Ns [N/mm]')
    plt.ylabel('Ny [N/mm]')
    plt.title('Failure Envelope (Ny-Ns)')
    plt.legend()
    plt.grid(True)
    plt.axis('equal')
    plt.show()

    # stack strains to make a nice txt file i can import into excel
    dataxy = np.hstack((
        fpfstrainsNxNy.T,
        lpfstrainsNxNy.T
    ))
    ...
    # Create header (12 columns total: 4 arrays × 3 components)
    headerxy = (
        "fpf_NxNy_1,fpf_NxNy_2,fpf_NxNy_3"
        "lpf_NxNy_1,lpf_NxNy_2,lpf_NxNy_3,"
    )
    headerys = (
        "fpf_NyNs_1,fpf_NyNs_2,fpf_NyNs_3,"
        "lpf_NyNs_1,lpf_NyNs_2,lpf_NyNs_3,"
    )

    # Save to file
    np.savetxt("strainsxy.txt", dataxy, delimiter=",", header=headerxy, comments="")
    np.savetxt("strainsys.txt", datays, delimiter=",", header=headerys, comments="")
